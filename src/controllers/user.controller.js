...
const changeCurrentPassword = asyncHandler(async (req, res) => {
  ...
  if (!isPasswordCorrect) {
    throw new ApiError(400, "Invalid old pswd");
  }
...
//middlware lgaya tha aur req.user=user inject krdia tha to simple
...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  if (!fullName || !email) {
    throw new ApiError(400, "Email or Password is/are missing");
  }
  const user = User.findByIdAndUpdate(
    ...
    { new: true } //it returns the new response
  ).select("-password");
...
//files update-->need middleware first i.e. multer
// and also those users can update who are already logged in
